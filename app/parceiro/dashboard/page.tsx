...
export default function ParceiroDashboardPage() {
  ...
  const {
    ...
    error,
    revalidate,
  } = useCachedFetch<ResumoPayload>('/api/parceiro/resumo', {
    ttl: 45,
  ...
  const handleCopyLink = async () => {
    if (!data?.parceiro.linkVenda) return
    try {
      await navigator.clipboard.writeText(data.parceiro.linkVenda)
      setLinkCopied(true)
  ...
  const comissaoMensalidadesLabel = useMemo(() => {
    if (!data) return ''
    const max = data.parceiro.comissaoMensalidadesMax
      ...
      <header className="sticky top-0 z-50 border-b border-gray-200 bg-white">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          <div className="min-w-0">
            ...
            {data?.parceiro && (
              <p className="text-xs text-gray-500 sm:text-sm">{data.parceiro.nome}</p>
            )}
          </div>
          ...
          <>
            ...
            <div className="rounded-lg bg-white p-6 shadow space-y-3">
              ...
              <div className="text-xs text-gray-500 space-y-1">
                <p>Código: <strong className="font-medium text-gray-700">{data.parceiro.codigoIndicacao}</strong></p>
                <p>Comissão: <strong>{data.parceiro.comissaoPercentualMensalidade}%</strong> sobre mensalidades pagas</p>
