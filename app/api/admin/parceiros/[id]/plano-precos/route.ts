...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const { data: planoPrecos, error } = await supabase
      .from('parceiro_plano_precos')
      .select('id, parceiro_id, plano_id, valor_por_pessoa, created_at, updated_at, planos(id, nome, codigo)')
    ...
    console.error('Admin parceiro plano-precos GET error:', error)
...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('parceiro_plano_precos')
      .upsert(upsertData, { onConflict: 'parceiro_id,plano_id' })
      .select('id, parceiro_id, plano_id, valor_por_pessoa, created_at, updated_at')
    ...
    console.error('Admin parceiro plano-precos PUT error:', error)
    return NextResponse.json({ error: 'Erro ao processar requisição.' }, { status: 500 })
  }
}
