...
// PATCH /api/admin/parceiros/[id]/planos/[planId] — update a plan
export async function PATCH(request: NextRequest, { params }: Params) {
  const auth = await requireAdminAuth(request)
  if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status })

  const { id: parceiroId, planId } = await params
  ...
  if (error) {
    console.error('parceiro_planos PATCH error:', error)
...
// DELETE /api/admin/parceiros/[id]/planos/[planId] — remove a plan
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  if (error) {
    console.error('parceiro_planos DELETE error:', error)
