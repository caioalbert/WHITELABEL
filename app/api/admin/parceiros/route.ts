...
async function generateUniqueParceiroCode(baseName: string) {
  const supabase = createAdminClient()
  const base = normalizeCodePart(baseName) || 'PARCEIRO'
...
function mapDatabaseErrorMessage(error: unknown) {
  ...
  if (/relation .*parceiros|does not exist|42P01/i.test(details)) {
    return 'Banco desatualizado. Execute scripts/015_add_parceiros_module.sql no Supabase SQL Editor.'
  }

  if (/duplicate key|parceiros_email|parceiros_codigo/i.test(details)) {
    return 'Já existe um parceiro com este e-mail ou código.'
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, parceiros: data || [] })
  } catch (error) {
    ...
    console.error('Admin parceiros GET error:', error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const semAdesao = body.semAdesao !== false // default: true (parceiros não cobram adesão)

    const parceiroId = crypto.randomUUID()
    ...
    if (createUserError || !createdUserData.user) {
      const message = createUserError?.message || 'Não foi possível criar usuário de acesso do parceiro.'
    ...
    const mappedMessage = mapDatabaseErrorMessage(error)
    if (mappedMessage) {
      const status = /Já existe um parceiro/i.test(mappedMessage) ? 409 : 500
    ...
    console.error('Admin parceiros POST error:', error)
